...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def union(that: TweetSet): TweetSet =
    // There might be some common elements between left and that, which can reduce the number of calls to incl()
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(x => google.exists(l => x.text.contains(l)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(x => apple.exists(l => x.text.contains(l)))
